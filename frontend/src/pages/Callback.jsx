    ...
    const initializeApp = async () => {
      try {
        ...
        if (!authStatusResponse.data.isAuthenticated) {
          ...
          const errorMsg = authStatusResponse.data.message || 'Authentication failed';
          const availableCookies = authStatusResponse.data.availableCookies || [];
          ...
          throw new Error(`Authentication failed: ${errorMsg}. Available cookies: ${availableCookies.join(', ')}`);
        }
        ...
        if (error.message.includes('cookies')) {
          errorMessage = 'Browser cookie issue detected. Please check your browser settings or try a different browser.';
